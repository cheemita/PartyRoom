...
<body>
  ...
  `;
          ...
          console.log({chairsPerUser}, {chairsSelected});
            if ($(this).attr("src") === "img/chair-occupied-user.png") {
                showModal('Remove my chair', 'Do you want to remove this chair?', "Yes, remove", "No, don't remove", () => {
                    ...
                    $.post('updateChairsAdminPersonal.php', { action: "delete", chair: $(this).attr("data-chair"), user_id: '<?php echo $_SESSION['id']; ?>'  }, function(result) {
                ...
                showModal('Reserve new chair', 'Do you want to reserve this chair?', "Yes, reserve", "No, don't reserve", () => {
                    ...
                    $.post('updateChairsAdminPersonal.php', { action: "update", chair: $(this).attr("data-chair"), user_id: '<?php echo $_SESSION['id']; ?>'  }, function(result)
                    {
                        chairsSelected++;
        ...
        async function getChairs() {
            let url = "code/chairsAdminPersonal.php";
            await fetch(url)
