...
// Create the "room1" table if it doesn't exist
$sql_create_table = "CREATE TABLE IF NOT EXISTS room1 (
...
if ($mysql->query($sql_create_table) === TRUE) {
    echo "Table 'room1' created successfully.<br>";
} else {
...
$sql_insert_data = "INSERT INTO room1 (idUser, numMesas, numInvitados, tipoFiesta, tipoComida)
        VALUES (?, ?, ?, ?, ?)";
...
if ($stmt->execute()) {
    ...
    header("Location: RoomAdmin2.php");
    exit;
} else {
